---
id: A100
title: Multi-agent release commander
plugin: gitops-release-controller
plugins: [gitops-release-controller]
mutation: apply
phase: 4
status: blueprint
...
# A100 — Multi-agent release commander
...
planner, CI, GitHub, observability, approvals

## Output and guardrail

Coordinated plan, checkpoints, final human release approval
...
4. **Approval:** required for production writes, external communications, deletes, migrations, or spending. Mutation class: `apply`.
...
1. Load `.cursor/skills/levelupworld/gitops-release-controller/SKILL.md` (and the catalog router if needed).
